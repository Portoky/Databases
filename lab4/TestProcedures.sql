...
BEGIN
	...
	Begin
		...
		Begin
			...
			If(@isPrimary = 1 AND @isForeign = 1) --in this case we need to check if the generated value is unique! I did not.
			Begin
			...
			End
			Else if @isPrimary = 0 AND @isForeign = 0 --it is not connected to anything just generate random values corresponding the datatype
			Begin
				If @DataType = 'int'
				Begin 
